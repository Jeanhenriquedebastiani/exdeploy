class ControleArCondicionado:
    ...
    def aumenta_temperatura(self):
        if self.ligado:
            if self.maxima == self.temperatura:
                print(f"Temperatura já esta no maximo: {self.temperatura}")
            else:
                self.temperatura += 1
    ...
    def diminui_temperatura(self):
        if self.ligado:
            if self.minima == self.temperatura:
                print(f"Temperatura já esta no minimo: {self.temperatura}")
            else:
                self.temperatura -= 1
